...
export class AvailabilityComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (loggedInUser) {
      try {
        ...
        console.log("Init: loggedInUser OK", p?.userId);
      } catch (e) {
        console.error("Init: Error parsing loggedInUser");
      }
    }
    if (userData) {
      try {
        const p = JSON.parse(userData);
        console.log("Init: userData OK", p?.userId);
      } catch (e) {
        console.error("Init: Error parsing userData");
      }
    }
    console.log("================================");
  ...
  loadAvailabilities(): void {
    ...
    this.interviewerService.searchAvailabilities().subscribe({
      next: (response: any) => {
        ...
        if (response && response.items && Array.isArray(response.items)) {
          ...
          console.log(
            "Found",
            itemsToProcess.length,
            "availability records in direct array"
          );
        } else {
          console.warn(
            "Unexpected API response format or empty response:",
            response
          );
          this.apiError =
            "Could not load availabilities: unexpected server response.";
      ...
      error: (error: any) => {
        ...
        this.isLoading = false;
        this.apiError =
          "Error loading availabilities from server. Please try again later.";
  ...
  private processAvailabilityData(data: any[]): void {
    console.log("Processing availability data with proper BST conversion...");
    if (!data || data.length === 0) {
      this.interviewerAvailability = [];
      return;
    }

    const groupedByBSTDate: Record<string, TimeSlot[]> = {};

    data.forEach((item) => {
      if (!item.startTime || !item.endTime) return;

      // Parse raw UTC ISO strings directly
      const startDateUtc = new Date(item.startTime);
      const endDateUtc = new Date(item.endTime);

      if (isNaN(startDateUtc.getTime()) || isNaN(endDateUtc.getTime())) {
        console.warn("Invalid date in item, skipping", item);
        return;
      }

      // Build the BST date‐key from the same UTC date
      const dateKey = startDateUtc.toLocaleDateString("en-CA", {
        timeZone: "Asia/Dhaka",
      }); // "YYYY-MM-DD"

      if (!groupedByBSTDate[dateKey]) {
        groupedByBSTDate[dateKey] = [];
      }

      groupedByBSTDate[dateKey].push({
        id: Math.random(),
        apiId: item.id,
        startTimeUTC: item.startTime,
        endTimeUTC: item.endTime,
        // Format *into* BST but without any manual offset hacks:
        startTime: this.formatBSTTime(startDateUtc),
        endTime: this.formatBSTTime(endDateUtc),
      });
    });

    // Map into Interviewer[] as before…
    this.interviewerAvailability = Object.entries(groupedByBSTDate)
      .map(([dateString, slots]) => ({
        name: "Current User",
        dateString,
        date: new Date(dateString + "T00:00:00+00:00"),
        timeSlots: slots,
        isAvailable: true,
        expanded: false,
      }))
  ...
  /** helper to format a UTC Date into BST display */
  private formatBSTTime(date: Date): string {
  ...
  formatTime(date: Date): string {
    ...
    try {
      return date.toLocaleTimeString("en-US", {
        // Use en-US locale for structure, override TZ
  ...
  generateCalendarDays() {
    ...
    const firstDayOfMonth = new Date(
      Date.UTC(this.currentYear, this.currentMonth, 1)
    );
    const daysInMonth = new Date(
      Date.UTC(this.currentYear, this.currentMonth + 1, 0)
    ).getUTCDate();
        ...
        if (weekIndex === 0 && dayIndex < startDayOfWeek) {
          ...
          const prevMonthDay = new Date(
            Date.UTC(
              this.currentYear,
              this.currentMonth,
              1 - (startDayOfWeek - dayIndex)
            )
          );
          week.push({
            date: prevMonthDay,
            dayNumber: prevMonthDay.getUTCDate(),
          ...
          const dateUTC = new Date(
            Date.UTC(this.currentYear, this.currentMonth, dayCounter)
          );
          // Disable past dates comparing UTC date with local 'today' start
          const isDisabled =
            dateUTC < today && dateUTC.toDateString() !== today.toDateString();
          week.push({
            date: dateUTC,
          ...
          week.push({
            date: nextMonthDay,
            dayNumber: nextMonthDay.getUTCDate(),
            isCurrentMonth: false,
            isDisabled: true,
          });
          dayCounter++;
  ...
  toggleDate(calendarDate: Date): void {
    ...
    if (
      this.selectedDate &&
      this.selectedDate.getTime() === clickedDateUTC.getTime()
    ) {
  ...
  addMoreTimeRange() {
    ...
    if (this.editingSlot) {
      alert(
        "You can only edit one time slot at a time. Save or cancel the current edit to add new ranges."
      );
      return;
    }
  ...
  editTimeSlot(interviewer: Interviewer, slotId: number) {
    ...
    if (slotIndex !== -1) {
      ...
      try {
        ...
        console.log(
          `Editing slot UI ID ${slotId} (API ID: ${slotToEdit.apiId}). Start: ${startHHMM}, End: ${endHHMM}. Original UTC: ${slotToEdit.startTimeUTC}`
        );
        window.scrollTo({ top: 0, behavior: "smooth" }); // Scroll to top form
      } catch (e) {
        console.error("Error parsing date for editing:", e);
        alert(
          "Could not prepare the slot for editing due to a date parsing error."
        );
  ...
  saveAvailability() {
    ...
    if (
      ...
      !this.timeRanges[0].startTime ||
      !this.timeRanges[0].endTime
    ) {
      alert(
        "Please ensure at least one valid time range (HH:MM format) is entered."
      );
      return;
    }
    ...
    const selectedDateStartOfDayLocal = new Date(
    ...
    for (const range of this.timeRanges) {
      ...
      if (
        !range.startTime ||
        !range.endTime ||
        !/^\d{2}:\d{2}$/.test(range.startTime) ||
        !/^\d{2}:\d{2}$/.test(range.endTime)
      ) {
        alert(
          `Invalid time format: ${range.startTime}-${range.endTime}. Please use HH:MM (24-hour) format.`
        );
        return;
      }
      ...
      if (
        ...
        alert(
          `Invalid time value: ${range.startTime}-${range.endTime}. Hours 0-23, Minutes 0-59.`
        );
        return;
      }
      ...
      if (endDateTimeLocal <= startDateTimeLocal) {
        alert(
          `End time (${range.endTime}) must be strictly later than start time (${range.startTime}).`
        );
        return;
      }
      ...
      if (
        ...
        startDateTimeLocal.toDateString() === now.toDateString()
      ) {
        // If it's today, check if the time itself is in the past/present
        if (
          startDateTimeLocal.getHours() < now.getHours() ||
          (startDateTimeLocal.getHours() === now.getHours() &&
            startDateTimeLocal.getMinutes() <= now.getMinutes())
        ) {
          alert(
            `Cannot set start time (${range.startTime}) in the past for today.`
          );
          return;
        }
      ...
      const diffInMinutes =
        (endDateTimeLocal.getTime() - startDateTimeLocal.getTime()) /
        (1000 * 60);
      if (diffInMinutes < 60) {
        // Example: Require at least 60 minutes
        alert(
          `Time range ${range.startTime} - ${range.endTime} must be at least 1 hour long.`
        );
        return;
      }
    }
    ...
    if (this.timeRanges.length > 1) {
      // No need to check overlap if only one range
      const timeIntervals = this.timeRanges
        .map((range) => {
          const start =
            parseInt(range.startTime.split(":")[0], 10) * 60 +
            parseInt(range.startTime.split(":")[1], 10);
          const end =
            parseInt(range.endTime.split(":")[0], 10) * 60 +
            parseInt(range.endTime.split(":")[1], 10);
      ...
      for (let i = 0; i < timeIntervals.length - 1; i++) {
        if (timeIntervals[i].end > timeIntervals[i + 1].start) {
          // Check if end overlaps next start
          alert(
            "Time ranges within your new selection cannot overlap. Please adjust."
          );
          return;
        }
      }
    ...
    const existingInterviewerData = this.interviewerAvailability.find(
      (i) => i.dateString === selectedDateString
    );
    let existingSlotsOnDate: TimeSlot[] = existingInterviewerData
      ? existingInterviewerData.timeSlots
      : [];

    // If editing, exclude the slot being edited from the overlap check list
    if (this.editingSlot) {
      const slotBeingEditedId =
        this.editingSlot.interviewer.timeSlots[this.editingSlot.slotIndex].id;
      existingSlotsOnDate = existingSlotsOnDate.filter(
        (slot) => slot.id !== slotBeingEditedId
      );
      console.log(
        "Excluding slot being edited (UI ID:",
        slotBeingEditedId,
        ") from overlap check."
      );
    }
    ...
    for (const newTimeRange of this.timeRanges) {
      if (
        ...
        alert(
          `Time slot ${newTimeRange.startTime}-${newTimeRange.endTime} overlaps with another existing availability for this date. Please adjust.`
        );
    ...
    const availabilitySlots: any[] = this.timeRanges.map((range) => {
    ...
    this.interviewerService.createAvailability(availabilitySlots).subscribe({
      ...
      error: (error: any) => {
        ...
        this.showToastNotification(
          `Error saving availability: ${errorMessage}`,
          "error"
        );
  ...
  showDeleteModal(interviewer: Interviewer, slotId: number) {
    ...
    if (slot) {
      console.log(
        `Attempting to delete slot UI ID: ${slot.id}, API ID: ${slot.apiId}`
      );
  ...
  confirmDelete() {
    if (this.selectedInterviewer && this.selectedSlotId !== null) {
      const slotToDelete = this.selectedInterviewer.timeSlots.find(
        (s) => s.id === this.selectedSlotId
      );

      if (!slotToDelete) {
        console.error("Cannot delete: Slot not found");
        this.showToastNotification(
          "Cannot delete this slot: Slot not found in local data.",
          "error"
        );
      ...
      if (!slotToDelete.apiId) {
        ...
        this.showToastNotification(
          "Cannot delete this slot: No server ID available.",
          "error"
        );
        this.closeDeleteModal();
        return;
      ...
      this.interviewerService.deleteAvailability(slotToDelete.apiId).subscribe({
        next: (response: any) => {
          ...
          this.isLoading = false;
          this.showToastNotification(
            "Availability time slot deleted successfully!",
            "success"
          );
        ...
        error: (err: any) => {
          ...
          this.isLoading = false;
          const errorMessage =
            err.error?.message || err.message || "Server error occurred.";
          this.showToastNotification(
  ...
  private checkOverlap(
    ...
    try {
      ...
      const newStartMinutes =
        parseInt(newStartHHMM.split(":")[0], 10) * 60 +
        parseInt(newStartHHMM.split(":")[1], 10);
      const newEndMinutes =
        parseInt(newEndHHMM.split(":")[0], 10) * 60 +
        parseInt(newEndHHMM.split(":")[1], 10);

      if (
        isNaN(newStartMinutes) ||
        isNaN(newEndMinutes) ||
        newStartMinutes >= newEndMinutes
      ) {
        console.error(
          "Invalid new time range format passed to overlap check:",
          newStartHHMM,
          newEndHHMM
        );
      ...
      return existingSlots.some((slot) => {
        ...
        try {
          ...
          if (
            ...
            throw new Error(
              "Invalid date parsed from existing slot UTC string"
            );
          }
        } catch (parseError) {
          console.warn(
            "Could not parse date from existing slot during overlap check:",
            slot,
            parseError
          );
        ...
        const slotStartMinutesUTC =
          existingStartDate.getUTCHours() * 60 +
          existingStartDate.getUTCMinutes();
        const slotEndMinutesUTC =
          existingEndDate.getUTCHours() * 60 + existingEndDate.getUTCMinutes();

        // Core overlap condition: (StartA < EndB) and (EndA > StartB)
        const overlaps =
          newStartMinutes < slotEndMinutesUTC &&
          newEndMinutes > slotStartMinutesUTC;

        if (overlaps) {
          console.log(
            `Overlap detected: New ${newStartHHMM}-${newEndHHMM} (mins ${newStartMinutes}-${newEndMinutes}) vs Existing UTC ${slot.startTimeUTC}-${slot.endTimeUTC} (UTC mins ${slotStartMinutesUTC}-${slotEndMinutesUTC})`
          );
        }
        return overlaps;
      });
